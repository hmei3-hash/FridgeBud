...
BLOCKLIST = [
    'person', 'human', 'people', 
    'bottle', 'cup', 'chair', 'table', 'desk',
    'monitor', 'laptop', 'keyboard', 'mouse',
    'wall', 'door', 'window', 'ceiling',
    'hand', 'face', 'head', 'body',
        ...
        if model_loaded and confirm_food is None:
            try:
                ...
                if not df.empty:
                    ...
                    # 组合置信度和面积：面积大的物体权重更高
                    df['score'] = df['confidence'] * (df['area'] / df['area'].max())
                    ...
                    for idx, (_, row) in enumerate(df.head(3).iterrows()):
                        print(f"  {idx+1}. {row['name']:15s} | 置信度: {row['confidence']:.3f} | 分数: {row['score']:.3f}")
                    
                    top = df.iloc[0]
                    
                    coco_label = top['name'].lower()
                    detection_confidence = float(top['score'])  # 使用加权分数
                    ...
                    for coco_key, pixel_key in COCO_TO_PIXEL.items():
                        if coco_key == coco_label:
                            if pixel_key in sprites:
                                pixel_food = pixel_key
                                print(f"✓ 精确匹配: {coco_label} -> {pixel_key}")
                                break
                    
                    # 方法2：包含匹配（coco_label 包含或被包含在 coco_key 中）
                    if not pixel_food:
                        for coco_key, pixel_key in COCO_TO_PIXEL.items():
                            if (coco_key in coco_label or coco_label in coco_key):
                                if pixel_key in sprites:
                                    pixel_food = pixel_key
                                    print(f"✓ 模糊匹配: {coco_label} -> {pixel_key}")
                                    break
                    
                    # 方法3：字母开头匹配
                    if not pixel_food:
                        for food in ALL_FOODS:
                            if food.lower().startswith(coco_label[:3]):
                                pixel_food = food
                                print(f"✓ 首字母匹配: {coco_label} -> {pixel_food}")
                                break
                    
                    # 调试信息
                    if not pixel_food:
                        print(f"⚠ 无法映射: {coco_label} (置信度: {detection_confidence:.2f})")
                    
                    if pixel_food and detection_confidence > 0.25:  # 降低阈值因为现在用的是加权分数
