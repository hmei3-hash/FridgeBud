...
# ── 防误识别参数（可调）──
CONF_THRESH          = 0.70   # 置信度阈值（0.82太严，真水果也过不了）
ENTROPY_MAX          = 2.8    # 熵上限（稍放宽）
STABLE_FRAMES_NEEDED = 3      # 连续帧要求（3帧 ~0.24s 足够滤掉闪烁）
SCENE_CHANGE_THRESH  = 12.0   # 场景变化阈值（用于建立基线）
IDLE_TIMEOUT         = 3.0    # 场景静止超过此秒数后回到"等待新物体"状态

_det_debug = {
    "raw_label":"", "raw_conf":0.0, "entropy":0.0,
    "stable_count":0, "scene_diff":0.0, "reject_reason":"", "phase":"idle"
}

def detection_thread():
    """
    两阶段检测：
      phase=idle   : 等待场景变化（有新物体进入画面）
      phase=active : 场景已变化，持续识别直到连续帧确认或超时回idle
    这样解决了"场景变化"和"连续帧"互相矛盾的问题。
    """
    stable_label = ""
    stable_count = 0
    prev_gray    = None
    phase        = "idle"       # idle / active
    baseline_gray = None        # 空闲时的背景基线
    active_timer  = 0.0         # active 阶段静止计时
    last_time     = time.time()
    ...
    while True:
        ...
        prev_gray = cur_gray
        _det_debug["scene_diff"] = mean_diff

        if phase == "idle":
            _det_debug["phase"] = "idle"
            # 等待场景变化：和基线比较
            if baseline_gray is None:
                baseline_gray = cur_gray.copy()

            base_diff = float(np.mean(cv2.absdiff(cur_gray, baseline_gray)))
            if base_diff > SCENE_CHANGE_THRESH:
                # 有新东西进入画面！切到 active
                phase = "active"
                active_timer = 0.0
                stable_label = ""
                stable_count = 0
                _det_debug["reject_reason"] = "检测到新物体, 开始识别..."
            else:
                # 持续更新基线（慢慢适应光照变化）
                baseline_gray = cv2.addWeighted(baseline_gray, 0.95, cur_gray, 0.05, 0)
                _det_debug["reject_reason"] = f"等待新物体(diff={base_diff:.1f})"
                _det_debug["stable_count"] = 0
                time.sleep(0.08)
                continue

        # ── phase == "active" ──
        _det_debug["phase"] = "active"

        # 如果画面持续静止太久（物体被拿走了），回到 idle
        if mean_diff < 3.0:
            active_timer += dt_t
        else:
            active_timer = 0.0

        if active_timer > IDLE_TIMEOUT:
            phase = "idle"
            baseline_gray = cur_gray.copy()
            stable_label = ""
            stable_count = 0
            _det_debug["reject_reason"] = "超时回到等待状态"
            time.sleep(0.08)
            continue

        # 推理
        label, conf, entropy = classify_frame(frame)
        _det_debug["raw_label"] = label
        _det_debug["raw_conf"]  = conf
        _det_debug["entropy"]   = entropy

        # 置信度过滤
        if conf < CONF_THRESH:
            stable_count = 0
            stable_label = ""
            _det_debug["reject_reason"] = f"置信度低({conf:.0%}<{CONF_THRESH:.0%})"
            _det_debug["stable_count"] = 0
            time.sleep(0.08)
            continue
        ...
        if entropy > ENTROPY_MAX:
            stable_count = 0
            stable_label = ""
            _det_debug["reject_reason"] = f"不确定(熵={entropy:.2f}>{ENTROPY_MAX})"
            _det_debug["stable_count"] = 0
            time.sleep(0.08)
            continue
...
while running:
    ...
    if current_tab=="scan":
        ...
        if state!=STATE_BARCODE:
            dbg=_det_debug; rl=dbg.get("raw_label",""); ph=dbg.get("phase","idle")
            phase_txt = f"[{ph}]" if ph else ""
            if rl:
                screen.blit(font_tiny.render(f"{phase_txt} {rl} {dbg['raw_conf']:.0%}  熵:{dbg['entropy']:.1f}  稳定:{dbg['stable_count']}/{STABLE_FRAMES_NEEDED}  diff:{dbg['scene_diff']:.1f}",True,theme.text_dim),(50,328))
            elif ph=="idle":
                screen.blit(font_tiny.render(f"{phase_txt} 等待新物体进入画面...",True,theme.text_dim),(50,328))
            rej=dbg.get("reject_reason","")
            if rej: screen.blit(font_tiny.render(f">> {rej}",True,(200,100,50)),(50,344))
            elif rl and not rej: screen.blit(font_tiny.render(">> 识别通过!",True,GREEN),(50,344))
        if state==STATE_IDLE:
