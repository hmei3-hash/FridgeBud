...
def draw_queue(surf, fruit_key, bag_cnt, queue_list):
    """
    Left column (50,370+): live bag preview while bagging
    Bottom strip (50,620): confirmed queue items after OK
    """
    # ── Confirmed queue strip at the bottom ──
    qx, qy = 50, 620
    surf.blit(font_medium.render("队列:", True, BLACK), (qx, qy))
    if queue_list:
        ix = qx + 55
        for (sk, cnt) in queue_list:
            if sk and sk in sprites:
                icon = pygame.transform.scale(sprites[sk], (38, 38))
                surf.blit(icon, (ix, qy - 8))
                ct = font_medium.render(f"×{cnt}", True, ORANGE_C)
                surf.blit(ct, (ix + 40, qy + 6))
                ix += 90
    else:
    ...
    for event in pygame.event.get():
        ...
        if ok_button.clicked:
            ...
            if state == STATE_DONE:
                ...
                # 合并同种水果
                merged = False
                for i, (sk, cnt) in enumerate(shopping_queue):
                    if sk == bag_sprite:
                        shopping_queue[i] = (sk, cnt + bag_count)
                        merged = True; break
                if not merged:
